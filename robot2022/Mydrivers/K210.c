...
uint8_t uart8_rxbuff[5];
uint8_t usart2_rxbuff[5];
uint8_t QR=0,TrueOne=0,block=1;
uint8_t lo1=0,lo2=0,lo3=0; //需先进行初始化
//开启中断
void K210Init(void)
{
	HAL_UART_Receive_IT(&huart2,(void *)&usart2_rxbuff[0],5);
	HAL_UART_Receive_IT(&huart8,(void *)&uart8_rxbuff[0],5);
...
{
	//extern uint8_t lo1,lo2,lo3; //需先进行初始化
	lo1=qrcode/100;
	lo2=(qrcode%100)/10;
	lo3=qrcode%10;
}
            ...
            color=(RxBuffer1[3]<<8)|RxBuffer1[2];
				...
				if(RxState1==3)		//检测是否接受到结束标志
				{
						if(RxBuffer1[RxCounter1-1] == 0x5B)
						{
									...
									for(int j=0;j<=2;j++)
									{
										if(k==0&&colorShu[j]>200)     //可以将判断次数调高一下
										{
											ColorXian=j+1;       //将学习颜色存储在colorYuan中
											k=1;                   //k值用于判断是否学习了颜色
											//printf("%d\r\n",k);    //这里应加个延时，用状态机
										}
										else if(k==1&&colorShu[j]>200)
										{
											if(color==ColorXian)
												TrueOne=1;  //需先设定为0
										}
